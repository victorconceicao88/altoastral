...
// Mock images (substitua pelos caminhos reais)
const foodImages = {
...
const InterfaceCliente = () => {
  ...
  const calculateTotal = () => {
    ...
    const deliveryFee = deliveryOption === 'entrega' ? 2.50 : 0;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const message = `*Novo Pedido Alto Astral*%0A%0A` +
        ...
        `*Endereço:* ${deliveryOption === 'entrega' ? customerInfo.address : 'Retirada'}%0A` +
        `*Itens:*%0A${cart.map(item => `- ${item.quantity}x ${item.name} (€${(item.price * item.quantity).toFixed(2)})`).join('%0A')}%0A` +
        `*Taxa de Entrega:* €${deliveryOption === 'entrega' ? '2.50' : '0.00'}%0A` +
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <div className="border-t pt-4 space-y-2">
              <div className="flex justify-between font-semibold">
                <span>Subtotal:</span>
                <span>€{(calculateTotal() - (deliveryOption === 'entrega' ? 2.50 : 0)).toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span>Taxa de Entrega:</span>
                <span>€{deliveryOption === 'entrega' ? '2.50' : '0.00'}</span>
              </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <motion.button
                ...
                className={`p-4 rounded-xl border-2 transition-all ${deliveryOption === 'retirada' ? 'border-astral bg-astral/10 text-astral' : 'border-gray-200 bg-white'}`}
              >
                <div className="flex flex-col items-center space-y-2">
                  <FiHome size={24} className={deliveryOption === 'retirada' ? 'text-astral' : 'text-gray-400'} />
              ...
              <motion.button
                ...
                className={`p-4 rounded-xl border-2 transition-all ${deliveryOption === 'entrega' ? 'border-astral bg-astral/10 text-astral' : 'border-gray-200 bg-white'}`}
              >
                <div className="flex flex-col items-center space-y-2">
                  <FiTruck size={24} className={deliveryOption === 'entrega' ? 'text-astral' : 'text-gray-400'} />
                  <span>Entrega (+€2.50)</span>
                </div>
              </motion.button>
            </div>
          ...
          <div className="flex justify-between sticky bottom-0 bg-white p-4 -mx-4 border-t">
            ...
            <motion.button
              ...
              disabled={!customerInfo.name || !customerInfo.phone || (deliveryOption === 'entrega' && !customerInfo.address)}
              className="bg-astral text-white px-6 py-3 rounded-lg hover:bg-astral-dark transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Finalizar Pedido
            </motion.button>
          </div>
